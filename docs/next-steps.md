...
1. **U-STATE-1 (content-only repopulation, C10)** — **GATED: PROCEED-WITH-AMENDMENTS** (`docs/specs/u-state-1-content-repopulation-review.md`). Open items closed by default (payload-primary diff + fallback; new pure `content-reconcile.ts`; whole-document-root replace v1; full SSR re-emit; mechanism-only + zone-node preservation). Amendments A1–A10 + adversarial pins ADV-1..5 authoritative. Decision row **U-STATE-1-CONTENT-REPOPULATION-GATE** (GATED). **U-STATE-1a spec DRAFTED** (`docs/specs/unit-u-state-1a-content-reconcile.md` — the pure reconciler: `reconcileContentRoots` + 4 types; §4 states 1–12; §5 F1–F6). **Next: TestWriter authors the red set from the U-STATE-1a spec** (AGENTS.md item 9 / RCA-1), then implementations for `unit-u-state-1b..1d.md` (host application; lifecycle state; regression) are spec'd in their turn.
