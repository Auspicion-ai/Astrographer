...
- **Unit U-H1 — the pure registry WRITE module `src/main/rag-store-registry-write.ts` (the registry hot-apply slice's FIRST unit; execution order U-H1 → U-H2 → U-H3 → U-H5 → U-H4 → U-H6 → U-H8; U-H2 (U-H2a runtime controller + U-H2b closure rewiring) + U-H3 (confirmatory, DELIVERED-BY-U-H2A) are LANDED 2026-09-08 — see the Unit U-H2 and Unit U-H3 DONE rows; Unit U-H5 (the teardown primitives) is LANDED too — see the Unit U-H5 DONE row; Unit U-H4 (the hot-remove drain-then-teardown) is LANDED too — see the Unit U-H4 DONE row; Unit U-H6 (the hot-rename drain-then-teardown) is LANDED too — see the Unit U-H6 DONE row; the next cycle is U-H7 (default reassignment)) (2026-09-08).** Spec
  ...
  `docs/decisions.md`; the slice is NOT complete — U-H1 + U-H2 (U-H2a + U-H2b) + U-H3 (confirmatory) + U-H5 (teardown) + U-H4 (hot-remove, drain-then-teardown) are landed (see the Unit U-H2, Unit U-H3, Unit U-H5, and Unit U-H4 DONE rows); U-H6..U-H8 are pending). Landed: ONE NEW pure node-testable module
