...
**IMMEDIATE STATE (2026-09-08): two Phase-2 slices share the tree; the cross-store fan-out slice is COMPLETE and the registry hot-apply slice has U-H1 + U-H2a landed.** (1) **Cross-store fan-out (`stores:"all"`)** — U-F1 `mergeStoreResults` / U-F2 `qualifyStoreResult` / U-F3 the `stores:"all"` schema + MCP/IPC wiring are ALL LANDED and SLICE-COMPLETE (see the U-F1/U-F2/U-F3 DONE rows below; slice live-scenario PARKED). (2) **Registry hot-apply** — **U-H1** (`src/main/rag-store-registry-write.ts`) is LANDED; **U-H2a — the runtime hot-apply controller (`src/main/rag-store-runtime.ts`) — is LANDED (2026-09-08: 38/38 via `tests/unit-h2-runtime-controller.test.ts`, typecheck + build clean; the three RCA-3 adversarial host fixes F-H2-1..3 = HOST-1a/1b/2/3 landed + regression-tested; blind-greens 28 PASS / 1 FAIL / 3 DEFERRED with the F8 FAIL (the §5.5 R-kind single-quote prose) reconciled in the doc-review `archive/reviews/2026-09-08-unit-h2a-doc-review.md`);** **U-H2b (the closure rewiring in `main.ts`/`mcp-server.ts` — B1–B13/M1–M4 + the A-P2-3 refresh-on-apply) is the NEXT cycle**; then U-H3 → U-H5 → U-H4 → U-H6 → U-H8 (+ U-H7 split out), per `docs/specs/registry-hot-apply-review.md` §2 D8 + the unit decomposition. **Trio VERIFIED GREEN 2026-09-08 (the current build state): 2609 pass / 41 skip (118 files), typecheck clean, build clean** (this is the U-H1 baseline; the landed U-H2a suite adds 38 to that count — not re-run here per the doc-review's read-only scope) — the earlier 4 live-ollama integration failures (`tests/embeddings-ollama-integration.test.ts`, `tests/embeddings-batch.test.ts` W2 LIVE) were environmental (no `embeddinggemma` served) and re-pass once ollama serves the model (verified this pass). **A fresh supervisor's next action: begin the U-H2b cycle — re-derive U-H2b's red set from `docs/specs/unit-h2-runtime-controller.md` §5.8–§5.9 (the closure rewiring B1–B13/M1–M4 + the A-P2-3 refresh), then red→green→adversarial→blind-greens→doc-review per RCA-1/2/3/4/6; do NOT claim U-H2 done until U-H2b lands.** The full F/H slice work was checkpointed in commit `5735a59` so U-H2 builds on a clean green base.
...
- **Unit U-H1 — the pure registry WRITE module `src/main/rag-store-registry-write.ts` (the registry hot-apply slice's FIRST unit; execution order U-H1 → U-H2a → U-H2b → U-H3 → U-H5 → U-H4 → U-H6 → U-H8; U-H2a (the runtime controller) is LANDED 2026-09-08, U-H2b (the closure rewiring) is the NEXT cycle) (2026-09-08).** Spec
  ...
  `docs/decisions.md`; the slice is NOT complete — U-H1 + U-H2a are landed (U-H2a: 38/38, 2026-09-08; its DONE row lands after U-H2b); U-H2b..H8 are pending). Landed: ONE NEW pure node-testable module
