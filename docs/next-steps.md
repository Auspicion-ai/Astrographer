...
**Remaining per-unit spec passes (the rest of the UI-overhaul work).** No unit beside U-STATE-1 has an umbrella gate artefact; each still needs its own `docs/specs/<unit>.md` + TestWriter red set before implementation (AGENTS.md item 9 / umbrella amendment A1). Tracked here so the list is not lost: **U-MENU-1** (File + View menu shell surface; needed before C13/C17), **U-SHELL-1** (layout model + zones + C9 layout slice), **U-SHELL-2** (appearance tokens + tri-state theme, C1), **U-SHELL-3** collapse (C5), **U-SHELL-4** drag/relocate + C11 empty-zone reveal, **U-SHELL-5** gutters (C7), **U-SHELL-6** hover affordance (C6), **U-SHELL-7** settings modal (C3; needs U-STATE-1), **U-SHELL-8** View-menu pane visibility (C13), **U-SHELL-9** top-bar tabs (C14), **U-EDIT-1** markdown/html toggle (C8; existing `editing-mode-toggle-review.md` may serve as its gate), **U-EDIT-2** undo/redo + history sub-pane (C16; needs U-STATE-1), **U-IMPORT-1** File→Import (C17), **U-PARITY-1..n** (C18 advanced search, C19 hover-preview, the PARTIAL closures, PG12/PG13 decisions). Each is gated/delegated in its own cycle, never bundled (RCA-2).
