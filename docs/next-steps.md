...
- **Unit U-H1 — the pure registry WRITE module `src/main/rag-store-registry-write.ts` (the registry hot-apply slice's FIRST unit; execution order U-H1 → U-H2 → U-H3 → U-H5 → U-H4 → U-H6 → U-H8; U-H2 (U-H2a runtime controller + U-H2b closure rewiring) + U-H3 (confirmatory, DELIVERED-BY-U-H2A) are LANDED 2026-09-08 — see the Unit U-H2 and Unit U-H3 DONE rows; Unit U-H5 (the teardown primitives) is LANDED too — see the Unit U-H5 DONE row; the next cycle is U-H4 (hot-remove, drain-then-teardown)) (2026-09-08).** Spec
  ...
  `docs/decisions.md`; the slice is NOT complete — U-H1 + U-H2 (U-H2a + U-H2b) + U-H3 (confirmatory) + U-H5 (teardown) are landed (see the Unit U-H2, Unit U-H3, and Unit U-H5 DONE rows); U-H6..U-H8 are pending). Landed: ONE NEW pure node-testable module
...
- **Unit U-H2 — the runtime hot-apply controller + the closure rewiring (the registry hot-apply slice's SECOND unit; execution order U-H1 → U-H2 → U-H3 → U-H5 → U-H4 → U-H6 → U-H8 (+ U-H7 split out) — the slice now has U-H1 + U-H2 + U-H3 (confirmatory, DELIVERED-BY-U-H2A) + U-H5 (teardown) LANDED; the next cycle is U-H4 (hot-remove)) (2026-09-08).** Split per RCA-5/D8 into U-H2a + U-H2b (ONE spec file `docs/specs/unit-h2-runtime-controller.md`).
