...
No handoff item is opened by this unit. The rich-text converter is built IN-HOUSE as `src/main/rich-decompose.ts` (Unit U2, the pure `decomposeRichHtml` module — the `provident-editable@0.1.0` import plan was replaced by the in-house build); it is consumed by the contenteditable UI,
NOT this unit.
