...
| **C15** | Documents gain a **directory/category structure** | **DONE (2026-09-11 U-D1..D7)** — local `documentId` is now path-qualified `/`-joined (`markdown-import.ts:323` `[...documentPath, base].join('/')`), the root node carries `documentPath` + `tags` (`:359`), and `rag.list_documents`/`rag.query`/`rag-stream` filters + `edit.set_doc_meta` are landed. (The pre-slice flat `documentId` = a sanitized basename, `/`→`-`, is SUPERSEDED per ui-overhaul §3.3/`document-directory-category-review.md` Q1.) A derived doc-nav **tree** UI (ui-overhaul G2 / U-D8, incl. a create-document/create-folder affordance) is the pending piece. The external Gnosis engine has `wikiId` + `tags: string[]` | Add a primary corpus-relative **path** (single-parent directory tree, preserved at import) + optional multi-valued **tags**, on document metadata. Drives a doc-nav **tree**, category-filtered listing/retrieval, and Gnosis wiki/tag mapping. **Data-model change — its own proposal gate + units** (see §3.3). |
...
| **C17** | An **Import** item in the **File menu** that opens a file browser; select **one or more `.md` files** *or* **a directory**, importing **all `.md` files in that directory** | No File menu, no dialog (`showOpenDialog` absent); import only via the MCP `edit.import_markdown` (files list) | Native **File → Import…** opens an Electron `showOpenDialog` with an `.md`/`.markdown` filter. **Multi-file upload is the DEFAULT on all platforms** (`['openFile','multiSelections']`); **macOS** may additionally allow a directory in the same dialog; **Windows/Linux** get a SEPARATE `Import folder…` item (`['openDirectory']`) for **directory bulk upload** (the combined file+directory picker is macOS-only — W1-N2). A directory expands to its `.md` files (recursive depth is a gate decision); the resolved list routes to the SAME `importMarkdownCorpus` application handler as `edit.import_markdown`. The corpusRoot stays server-fixed (Unit MS4/ADV-1). See §4/G2 + §5.1 PG2. |
...
| **Q17** | Import directory scope (C17): recursive or top-level only; hidden dirs; `.markdown` too; symlinks; cap on file count | Top-level `.md` (+ `.markdown`) only, non-recursive v1 unless nesting is wanted; skip dot-dirs; no symlink follow; a sane file-count cap with a fail-loud message — **LANDED 2026-09-14 (U-IMPORT-1 DONE): `expandImportDirectory` top-level non-recursive, dot/symlink-skipped, `MAX_IMPORT_FILES=512` fail-loud** |
| **Q18** | Import menu placement: also an in-pane Import control in G2, or File-menu only? | Both — File menu is the primary; the G2 doc-nav pane may also expose an Import control (same handler) — **File-menu primary LANDED 2026-09-14 (U-IMPORT-1 DONE); the optional in-pane G2 Import control is a separate G2/doc-nav concern (NOT part of U-IMPORT-1)** |
...
12. **U-IMPORT-1 — File → Import…** (C17; needs U-MENU-1; platform-aware `showOpenDialog` — **multi-file `['openFile','multiSelections']` default on all platforms**; macOS may also allow a directory in the same dialog; **Windows/Linux add a separate `Import folder…` `['openDirectory']`** for directory bulk (W1-N2); `.md` filter + directory→`.md` expansion + import IPC → the shared `importMarkdownCorpus` handler) — **DONE (2026-09-14)**; spec `docs/specs/unit-u-import-1-import-surface.md`.
