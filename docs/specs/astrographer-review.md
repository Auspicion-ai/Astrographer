...
**Material-state clarification (2026-08-26, user ruling):** the N duplicates are PER-RENDER materializations, and render is TRANSIENT. In the current single-document view, only ONE document renders at a time, so only ONE instance of a shared subtree exists at any moment — the duplicate for whichever document is currently on screen (documents cannot loop in one view). The N duplicates COEXIST only if multiple documents render simultaneously, which the current UI cannot do. That requires **document tabs** (a pending feature — `docs/pending.md`). So "materializing in multiple document flows" means: the RAG node is materialized as N distinct provident subtrees, one per document flow, but only ONE is live at a time in the single-view UI; the N coexist only under a multi-document (tabs) render.
