...
export interface EngineRagStoreOptions {
  ...
  auth?: {
    token?: string
    /** TLS options (ca/cert/key) for the loopback connection. ACCEPTED + applied
     *  (Unit shell-integration §5.1) via `createEngineFetch(auth)` — the undici
     *  dispatcher (an Agent with connect: { ca, cert, key }). */
...
// ---------------------------------------------------------------------------
// The fetch-based SSE client factory (Unit shell-integration §5.2).
// ---------------------------------------------------------------------------
...
export function createSseClient(auth?: { token?: string }): SseClient {
  return {
    subscribe(url, handlers) {
      ...
      // A consumer-initiated teardown (close()) is NOT an onClose — after
      // close() the client fires NO further callbacks.
      let closed = false

      fetch(url, {
        method: 'GET',
        // Unit shell-integration §5.2 — the SSE GET carries ONLY the
        // Authorization: Bearer header when auth.token is set and NO
        // content-type header (a GET stream needs none). It must NOT reuse the
        // shared headers(auth) helper, which always adds content-type.
        ...
        (res) => {
          if (closed) return
          if (!res.ok) {
            handlers.onError(new Error('sse: non-2xx response'))
            return
          }
...
export function createEngineRagStore(opts: EngineRagStoreOptions): EngineRagStore {
  ...
  assertLoopback(opts.baseUrl, 'engine rag store')
  const baseUrl = opts.baseUrl.replace(/\/+$/, '')
  const fetchImpl = opts.fetch ?? createEngineFetch(opts.auth)
  const sse = opts.sse ?? createSseClient(opts.auth)
  ...
  const getEngineStatus = async (): Promise<HealthReport> => {
    ...
    let res: Response
    try {
      res = await fetchWithTimeout(fetchImpl, url, { method: 'GET', headers: headers(auth) }, requestTimeoutMs)
    } catch (err) {
