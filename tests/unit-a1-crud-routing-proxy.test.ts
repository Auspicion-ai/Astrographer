...
// ---------------------------------------------------------------------------
// Adversarial regression tests (RCA-3 HOST-MINOR findings, fixed + pinned).
// ---------------------------------------------------------------------------

describe('adversarial regressions (RCA-3)', () => {
  it('HOST-MINOR 1: the hex IPv4-mapped loopback form (::ffff:7f00:1) is accepted', () => {
    expect(() =>
      createEngineCrudRagStore({ baseUrl: 'http://[::ffff:7f00:1]:8080' }),
    ).not.toThrow()
  })

  it('HOST-MINOR 2: a Document body with a MISSING author key → EngineError (502)', async () => {
    const missingAuthor = { ...DOC_WIRE }
    delete (missingAuthor as any).author
    const store = crudStoreWithResponse(
      jsonResponse(
        crudResponseEnvelope({ method: 'createDocument', result: missingAuthor }),
      ),
    )
    const err = await captureError(
      store.createDocument({
  ...
  it('HOST-MINOR 3: a Document body with an ARRAY graph → EngineError (502)', async () => {
    const arrayGraph = { ...DOC_WIRE, graph: [] }
    const store = crudStoreWithResponse(
      jsonResponse(
        crudResponseEnvelope({ method: 'createDocument', result: arrayGraph }),
      ),
    )
    const err = await captureError(
      store.createDocument({
